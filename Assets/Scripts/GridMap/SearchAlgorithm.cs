...
namespace GridMap
{
    ...
    public static class SearchAlgorithm
    {
        public static IEnumerable<Vector2Int> DirectionEight(Vector2Int basePosition)
        {
            ...
            yield return basePosition + Vector2Int.up;
            yield return basePosition + Vector2Int.up + Vector2Int.left;
            yield return basePosition + Vector2Int.left;
        ...
        /// <param name="nextPoint">ある座標から1ステップで移動可能な相対座標を示す</param>
        /// <param name="function">算出された距離に応じた値で行列を生成するための関数</param>
        public static GridMap<T> DistanceMap<T>(Vector2Int basePosition, NextPoint nextPoint, DistanceMapFunction<T> function) where T : IConvertible =>
            DistanceMap(DefaultRange, basePosition, nextPoint, function);
        
        /// <summary>
        /// judgePositionがマップの範囲内であればtrueを返す。
        /// </summary>
        /// <param name="gridMap">範囲(Range)をとるマップ</param>
        /// <param name="judgePosition">次判定する座標</param>
        /// <returns>trueなら範囲内、falseならOutOfRange</returns>
        public static bool WithinMapRange<T>(this GridMap<T> gridMap, Vector2Int judgePosition) where T:IConvertible =>
            (judgePosition.x >= 0) && (judgePosition.y >= 0) && (judgePosition.x < gridMap.Range.x) &&
            (judgePosition.y < gridMap.Range.y);
